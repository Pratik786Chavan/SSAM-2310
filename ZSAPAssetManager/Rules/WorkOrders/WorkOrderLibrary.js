...
export class WorkOrderEventLibrary {
            ...
            switch (name) {
                ...
                case 'TypeLstPkr':
                    return libWoControls.updatePriority(context).then(() => {
                        // Update Maint Activity Type on Order Type Change
                        return libWoControls.zupdateMaintainanceActivityType(context).then(() => {
    ...
    static setDefaultValues(pageProxy, onCreate, onFollowUp) {
        ...
        //Set MaintActivityType based on deafult OrderType
        let maintTypePkr = pageProxy.getControl('FormCellContainer').getControl('MaintActivityTypeLstPkr');
        if (!ValidationLibrary.evalIsEmpty(pageProxy.binding.OrderType)) {
            var maintCtrlSpecifier = maintTypePkr.getTargetSpecifier();
            maintCtrlSpecifier.setQueryOptions(`$filter=OrderType eq '${pageProxy.binding.OrderType}'&$orderby=ActivityType`);
            return maintTypePkr.setTargetSpecifier(maintCtrlSpecifier).then(() => {
                return Promise.resolve(true);
            });
...
export class WorkOrderControlsLibrary {
    ...
    static zupdateMaintainanceActivityType(pageProxy) {
        try {
            let formCellContainer = pageProxy.getControl('FormCellContainer');
            let orderTypeValue = libCommon.getListPickerValue(formCellContainer.getControl('TypeLstPkr').getValue());
            let maintActivityControl = formCellContainer.getControl('MaintActivityTypeLstPkr');
            var maintCtrlSpecifier = maintActivityControl.getTargetSpecifier();

            maintCtrlSpecifier.setQueryOptions(`$filter=OrderType eq '${orderTypeValue}'&$orderby=ActivityType`);
            return maintActivityControl.setTargetSpecifier(maintCtrlSpecifier).then(() => {
                return Promise.resolve(true);
            });

        } catch (err) {
            //Implementing our Logger class
            Logger.error(pageProxy.getGlobalDefinition('/SAPAssetManager/Globals/Logs/CategoryWorkOrders.global').getValue(), `update Maint.  Activity Type Error: ${err}`);
            return Promise.resolve(false);
        }
    }
